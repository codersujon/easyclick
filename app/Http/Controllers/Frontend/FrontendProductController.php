...
class FrontendProductController extends Controller
{
    ...
    public function showProduct(string $slug)
    {
        ...
        // $related_products = Product::where('category_id', $product->category_id)->where('id', '!=', $product->id)->where('status', 1)->where('is_approved', 1)->inRandomOrder()->limit(4)->get();
        $related_products = Product::where('category_id', $product->category_id)->where('id', '!=', $product->id)->where('status', 1)->where('is_approved', 1)->inRandomOrder()->get();
        // dd($related_products);
        return view('frontend.pages.product-detail', compact('product', 'reviews', 'related_products'));
    }

    /**
     * Summary of chageListView
     * @param \Illuminate\Http\Request $request
     * @return void
     */
