...
class ReviewController extends Controller
{
    ...
    public function pendingOrder(){
        $data['pendingOrder'] = Order::where(['order_status'=> 'pending','user_id' => Auth::user()->id])
        ->with('orderProducts')  
        ->get();
        $data['title'] = 'Pending Order';
        // return "hello world ";
        //  dd($data['completedOrders']);
        return view('frontend.dashboard.complete_order.complete_order',$data);
    }

    public function CompleteOrder(){
        $data['pendingOrder'] = Order::where(['order_status'=> 'delivered','user_id' => Auth::user()->id])
        ->with('orderProducts')  
        ->get();
