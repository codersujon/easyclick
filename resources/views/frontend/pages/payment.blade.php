                ...
                <div class="row">
                    <div class="col-xl-3 col-lg-3">
                        <div class="wsus__payment_menu" id="sticky_sidebar">
                            <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist"
                                ...
                                {{-- get modal data from paypal --}}
                                <?php
                                    // $paypal_status = \App\Models\PaypalSetting::where('status', 1)->first()
                                    $paypal_status = \App\Models\PaypalSetting::first()->status == 1 ? 'Enabled' : 'Disabled';
                                    $stripe_status = \App\Models\StripeSetting::first()->status == 1 ? 'Enabled' : 'Disabled';
                                    $razorpay_status = \App\Models\RazorpaySetting::first()->status == 1 ? 'Enabled' : 'Disabled';
                                    $cod_status = \App\Models\CodSetting::first()->status == 1 ? 'Enabled' : 'Disabled';
                                ?>
                                @if ($cod_status == 'Enabled')
                                    <button class="nav-link common_btn @if($cod_status == 'Enabled') active @endif"
                                        id="v-pills-profile-tab" data-bs-toggle="pill"
                                        data-bs-target="#v-pills-cod" type="button" role="tab"
                                        aria-controls="v-pills-stripe" aria-selected="false">COD</button>
                                @endif
                                @if ($paypal_status == 'Enabled')
                                    <button class="nav-link common_btn @if($paypal_status == 'Enabled' && $stripe_status == 'Disabled' && $razorpay_status == 'Disabled' && $cod_status == 'Disabled') active @endif"
                    ...
                    <div class="col-xl-5 col-lg-5">
                        <div class="tab-content" id="v-pills-tabContent" id="sticky_sidebar">
                            ...
                            <div class="tab-pane fade @if($paypal_status == 'Enabled' && $stripe_status == 'Disabled' && $razorpay_status == 'Disabled' && $cod_status == 'Disabled')show active @endif"
